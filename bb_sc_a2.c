...
Lifetime (including blackbody radiation decay) calculation 
for a Rydberg state ns1/2 given by the sum over all n->n2 
dipole allowed matrix elements 

Radiative decay=32 PI*fs ^3/(6 Rr c^3)|<n,l,j|r|n2,k,j2>|²(En-En2)²(En-En2)
Blackbody decay=32 PI*fs ^3/(6 Rr c^3)|<n,l,j|r|n2,k,j2>|²(En-En2)²(En-En2)//(exp((pc*w)/(kb*temp))-1)
...
int main(void)
{
	...
	double j, j2, qd, qd2, En,En2, nqd, nqd2, dip, temp; //Tot angular mom, Energy of state with principal quantum number n
	char name[30], name2[30]; //Name for text file where eigenenergies will be saved
	double x, xs, xs2, xmin, mon, h, Xk[max], X[max], xvec[max], xveck[max], f1, f2, f3, f1k, f2k, f3k;
	...
	double  decay, decay1, decay2, decay3, decay4; //Decay variables
	double am1, am2,am3, am4, dipnn[max], ae, ae2;  //Angular momentum constant and matris element vector 
	double w, ang, nw[max], dvec[max];
	int q, v=0, t=1;
	...
	//---Loop for computing A coeffients---\\
	...
	for(p=0; p<2; p++)
	{
		...
		am1=k;
		am2=2*j2+1;
		am3=gsl_sf_coupling_6j(2*l, 2, 2*k, 2*j2, 1, 2*j);
		ang=am1*am2*am3*am3;
		deg=1;
		...
		nqd2=0;
		En2=0;
		ae=1.33333*fs/(csi*csi);
		
		for(n2=5; n2<n; n2++) //A coefficient calculation for n2<n
		{
			...
			w=2*Rr*csi*(En-En2); //Frequency n->n2			
			nw[n2-5]=1/(exp((pc*w)/(kb*temp))-1); //Black body occupation number for w
			dipnn[n2-5]=ae*deg*ang*pow((dme(X, Xk, xveck, nc, nck, N, N2, t)*aB),2)*(pow(2*PI*w,3)); //A coefficient for <n|r|n2>
			//printf("<%d|r|%d>=%lf, %3.6le, %3.10le, n(w)=%lf\n", n, n2, dme(X, Xk, xveck, nc, nck, N, N2, t), dipnn[n2-5],  w, nw[n2-5]);
		}
		
		

		for(n2=n; n2<n*1.8; n2++) //A coefficient calculation for n2>n
		{
			...
			nck=norm(Xk, xveck, N2); //Normalization constant for wavefunction 
			w=2*Rr*csi*(En2-En); //Frequency n->n2	
			nw[n2-5]=1/(exp((pc*w)/(kb*temp))-1); //Black body occupation number vector for w
			dipnn[n2-5]=ae*deg*ang*pow((dme(Xk, X, xvec, nc, nck, N2, N, t)*aB),2)*(pow(2*PI*w,3));  //A coefficient for <n|r|n2>
			//printf("<%d|r|%d>=%lf, %3.6le, %3.10le, n(w)=%lf\n", n, n2, dme(Xk, X, xvec, nc, nck, N2, N, t), dipnn[n2-5], w, nw[n2-5]);
		}
	...
	for(i=0; i<n; i++) //Sum of every dipole <n|z|n2> contribution from avec[n2-5] vector element 
	{
		dvec[i]=dipnn[i];
		decay1=decay1+dipnn[i];
		decay2=decay2+(dipnn[i]*nw[i]);
		//printf("%d\t%lf\n", i, dvec[i]);
	}
	i=n;
	for(i=n; i<1.8*n-5; i++)
	{
		dvec[i]=dipnn[i];
		decay3=decay3+(dipnn[i]*nw[i]);
		//printf("%d\t%lf\n", i, dvec[i]);
	}
	
	
	decay=decay1+decay2+decay3;
	decay4=decay2+decay3;
	
	printf("\nTotal decay=%1.5lf\nRadiative=%1.5lf\nBlacbody=%1.5lf\n", decay, decay1, decay4);
	
	sprintf(name, "%3.1lfK_%ds%1.1lfLifetime", temp, n, j2); //Name assignment 
	
	funguarda(dvec, nw, name, n*1.8); //Saves dvec and nw in a file
